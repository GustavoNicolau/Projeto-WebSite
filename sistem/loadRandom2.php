
<div class="sub-header col-md-12 col-xs-12">
    Veja mais aqui 
</div>

<div class="see-also-wrapper">


<?php 

try{

$sql = $cox->query("select * from produto ORDER by rand() limit 4 ");

foreach($sql as $row)
{

    $sql1 = $cox->query('select * from fotoproduto where id_produto = '.$row[0].' limit 1');
    foreach ($sql1 as $row1)
    {
        echo '
        <div  class="see-also-container col-md-3 col-xs-6">
            <a href="detail.php?prod='.$row[0].'">
                <img src="img/'.$row[0].'/'.$row1[2].'" border="0" class="img-responsive">
            </a>

            <a href="detail.php?prod='.$row[0].'" class="no-mobile"> 
                <h3 class="h5"> '.$row[1].' </h3>
            </a>
        </div>
     
        ';
    }
}
}catch(Exception $e){
    
}
?>

</div>